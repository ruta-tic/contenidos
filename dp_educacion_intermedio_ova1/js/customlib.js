...
(function(app) {
    ...
    /**
     * To handle when an activity has restarted.
     * @param {event} event
     * @param {JQuery object} $el
     * @param {object} args
     */
    function onActivityAgain(event, $el, args) {
        if (/1-caso/.test(args.id)) {
            $el.find('.wf-content').hide();
            $el.trigger('showPage', [0, false]);
            return;
        }
    }

    function onActivityLoadData(event, $el, args) {
        if (/2-OCDE/.test(args.id)) {

            var campo = 0, area = 0, disciplina = 0;

            try {
               var values = JSON.parse(args.data);
            }
            catch(e) {
               return;
            }
            ...
            app.selectvalues(campo, area, disciplina);
            return;
        }
    }
    ...
    $(app).on('jpit:activity:loaddata', onActivityLoadData);

    $( document ).ready(function() {
        $('.selection-check').on('click', function() {
            var $this = $(this);
            var $parent = $this.parent();

            var $selected = $parent.find('li.selected');
            var feedback = $selected.attr('data-feedback');

            if (feedback == '' || typeof feedback == 'undefined') {
                return;
            }

            var $wf = $parent.find('.wf-content');

            if ($wf.length == 0) {
                $wf = $('<div class="wf-content autox autoy"><p>' + feedback + '</p></div>');
                $parent.append($wf);
            }
            else {
                $wf.empty().append('<p>' + feedback + '</p>');
            }


            if ($selected.attr('data-correct') == 'true') {
                var $continue = $('<button class="general continue">Continuar</button>');
                $continue.on('click', function() {
                    $('#pag-resuelve .ctrl-pagination .next').trigger('click');
                });
                ...
                $close.on('click', function() {
                    $wf.hide({ effect: 'slide', direction: 'down' });
                });
        ...
        var $listcampos = $('#list-campos');
        var $listareas = $('#list-areas');
        var $listdisciplina = $('#list-disciplina');

        $listcampos.selectmenu({
            change: function(event, ui) {
                loadareas();
            }
        });

        $listareas.selectmenu({
            change: function() {
                loaddisciplinas();
            }
        });

        $listdisciplina.selectmenu();

        function loadcampos() {
            $.each(campos, function(i, v) {
                var $option = $('<option value="' + i + '">' + v.campo + '</option>');
                $listcampos.append($option);
            });
            $listcampos.selectmenu('refresh');
            loadareas();
        }

        function loadareas() {

            var val = $listcampos.val();
            var areas = campos[val].areas;

            $listareas.empty();
            $.each(areas, function(i, v) {
                var $area = $('<option value="' + i + '">' + v.nombre + '</option>');
                $listareas.append($area);
            });
            $listareas.selectmenu('refresh');
            loaddisciplinas();
        }

        function loaddisciplinas() {
            var campo = $listcampos.val();
            var area = $listareas.val();
            var disciplinas = campos[campo].areas[area].disciplinas;

            $listdisciplina.empty();

            $.each(disciplinas, function(i, v) {
                var $disciplina = $('<option value="' + i + '">' + v + '</option>');
                $listdisciplina.append($disciplina);
            });

            $listdisciplina.selectmenu('refresh');
        }

        function selectvalues(campo, area, disciplina) {
            $listcampos.val(campo);
            $listcampos.selectmenu('refresh');
            ...
            $listdisciplina.val(disciplina);
            $listdisciplina.selectmenu('refresh');
        }

        loadcampos();

        app.selectvalues = selectvalues;
...

    {
        ...
        areas: [
            ...
            disciplinas: ["Ciencias del cuidado de la salud y servicios (administración de hospitales, financiamiento)", "Políticas de salud y servicios", "Enfermería", "Nutrición y dietas", "Salud pública", "Medicina tropical", "Parasitología", "Enfermedades infecciosas", "Epidemiología", "Salud ocupacional", "Ciencias del deporte", "Ciencias socio biomédicas (planificación familiar, salud sexual, efectos políticos y sociales de la investigación biomédica)", "Ética", "Abuso de sustancias"]},
