...
jpit.activities.sortable.activity = function (container) {
    ...
    function randomize($list) {
        if ($list.attr('data-shuffle') === 'false') return;
        $list.find('[data-position]').sort(jpit.activities.sortable.utility.randOrder).appendTo($list);
    }
    ...
    var obj = {
        ...
        "enable" : function() {
            $.each(obj.list_items, function() {
                var $list_item = $(this);
                randomize($list_item);
                $list_item.sortable( "option", "disabled", false );
            });
        },
